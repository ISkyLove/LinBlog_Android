 ...
 */
public class FlexibleSpaceWithImageWithViewPagerTabActivity extends ObserverBaseActivity {
    ...
    private void translateTab(int scrollY, boolean animated) {
        ...
        ViewCompat.setTranslationY(overlayView, ScrollUtils.getFloat(-scrollY, minOverlayTransitionY, 0));
        ViewCompat.setTranslationY(imageView, ScrollUtils.getFloat(-scrollY / 2, minOverlayTransitionY, 0));

        // Change alpha of overlay
        ViewCompat.setAlpha(overlayView, ScrollUtils.getFloat((float) scrollY / flexibleRange, 0, 1));
        ...
        ViewCompat.setPivotY(titleView, 0);
        ...
        ViewCompat.setTranslationY(titleView, titleTranslationY);

        // If tabs are moving, cancel it to start a new animation.
        ViewCompat.animate(mSlidingTabLayout).cancel();
        ...
        if (animated) {
            ...
            ViewCompat.animate(mSlidingTabLayout)
                    .translationY(translationY)
                    .setDuration(200)
                    .start();
        } else {
            // When Fragments' scroll, translate tabs immediately (without animation).
            ViewCompat.setTranslationY(mSlidingTabLayout, translationY);
        }
    }
